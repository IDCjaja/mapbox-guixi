      ...
      function createCluster() {
        _this.markers = new L.MarkerClusterGroup({
          ...
          iconCreateFunction: function(cluster) {
            var clusterList = cluster.getAllChildMarkers();
            var tagArr = [];
            var tagColor = [];
            clusterList.forEach(item => {
              tagArr.push(item.options.tagId);
              tagColor.push(item.options.color);
            })
            var temp = [];
            var obj = {};
            ...
            tagColor = [...new Set(tagColor)];
            var data = [];
            for (var key in obj) {
              data.push({value:obj[key]})
            }
            //console.log(data);
            setTimeout(() => {
              var list = document.querySelectorAll('div.leaflet-marker-icon.leaflet-div-icon');
              list.forEach(function(el, index){
                  var myChart = echarts.init(list[index]);
                  var option = {
                    series : [
                      {
                        type: 'pie',
        ...
        _this.markerList.forEach(item => {
          var category = _this.existedCategories.find(catgy => item.categoryId === catgy.iconId );
          var tag = _this.existedTags.find(t => item.tagId === t.id);
          var marker = L.marker(new L.LatLng(item.latitude, item.longitude),{
            id: item.id,
            icon: L.mapbox.marker.icon({'marker-symbol': _this.categorySvgMap[category.iconId-1], 'marker-color': tag.color}),
            name: item.name,
